...
BANNED_IPS = (
    set(os.getenv('BANNED_IPS', '').split(','))
    if os.getenv('BANNED_IPS')
    else set()
)
...
R2_BUCKET_NAME = (
    os.getenv('R2_BUCKET_NAME', 'my-media-downloader').strip().strip('/')
)
...
cleanup_thread = threading.Thread(
    target=auto_cleanup_downloads_folder, daemon=True
)
...
def fetch_po_token():
  ...
  try:
    ...
    if os.path.exists(js_script):
      res = subprocess.run(
          ['node', js_script], capture_output=True, text=True, timeout=10
      )
      if res.returncode == 0:
...
def get_base_ydl_opts(url=None):
  ...
  # چارەسەرکردنی کێشەی دابەزاندنی فەیسبوک لە موبایل بە گۆڕینی User-Agent و هێدەرەکان
  headers = {
      'User-Agent': (
  ...
  if url:
    if 'instagram.com' in url:
      ...
      # زیادکردنی پشتگیری بۆ داواکارییەکانی فەیسبوک لەسەر موبایل و دیسکتۆپ
      headers['User-Agent'] = (
  ...
  opts = {
      ...
      'concurrent_fragment_downloads': (
          4
      ),  # زیادکردنی خێرایی دابەزاندن بە پارچەکردن
      'retries': 10,
      'fragment_retries': 10,
  }
...
def serve_downloaded_file(filename):
  return send_from_directory(
      DOWNLOAD_DIR, filename, as_attachment=True, download_name=filename
  )
      ...
      with yt_dlp.YoutubeDL(check_opts) as ydl:
        ...
        if dur > 3600:
          return (
              jsonify({
                  'error': (
                      'ڤیدیۆکە لە ١ کاتژمێر درێژترە و ناتوانرێت بە تەواوی'
                      ' دابەزێنرێت. دەتوانیت بە بەکارهێنانی تایبەتمەندی بڕین'
                      ' (Start/End Time) بەشێکی دابەزێنیت.'
                  )
              }),
              400,
          )
